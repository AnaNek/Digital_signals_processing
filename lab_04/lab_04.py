...
from scipy import signal, linspace
...
def main():
    ...
    s_gauss = GaussSignal(amplitude, SIGMA)
    ...
    butterworth_f = butterworth_filter(6, 20, 'low')
    gauss_f = gauss_filter(4, 20, 'low')

    pyplot.plot(t_values, gauss_values, 'r',
                t_values, values_w_gauss, 'g', t_values,
                values_w_impulse, 'b')
    pyplot.show()

    pyplot.plot(t_values, gauss_values, 'r',
                t_values, signal.filtfilt(gauss_f, 1, values_w_gauss), 'g',
                t_values, signal.filtfilt(butterworth_f, 1, values_w_gauss), 'b')
    pyplot.show()

    pyplot.plot(t_values, gauss_values, 'r',
                t_values, signal.filtfilt(gauss_f, 1, values_w_impulse), 'g',
                t_values, signal.filtfilt(butterworth_f, 1, values_w_impulse), 'b')
    pyplot.show()
